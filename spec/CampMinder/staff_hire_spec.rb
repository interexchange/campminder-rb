...
describe "StaffHire" do
  let(:staff_hire) { CampMinder::StaffHire.new(@data) }
  ...
  <staffDataArray>
    ...
    @payload_without_salary = %{<staffDataContainer version="1">
  <cmClientId>#{@data["client_id"]}</cmClientId>
  <token>#{@data["token"]}</token>
  <partnerClientId>#{@data["partner_client_id"]}</partnerClientId>
  <staffDataArray>
  ...
  describe "#payload" do
    ...
    context "with base_salary populated" do
      it "generates an xml payload" do
  ...
  describe "#post" do
    ...
    it "uses a proxy url if CAMPMINDER_PROXY_URL is set" do
      stub_const("CampMinder::PROXY_URL", "http://proxy.example:3128")
